...
*   **Database:** add summary metadata columns if not already present (`pdf_url`, `ai_summary`, `summary_generated_at`, processing status).
*   **External systems:** Cloudinary (file storage), Gemini API (AI summary), BullMQ + Upstash Redis (job queue).
